...
let e = world.afterEvents.dataDrivenEntityTrigger.subscribe((data) => {
  if (!(data.entity instanceof Player)) return;
  if (data.eventId != "rubedo:becomeAdmin") return;
  ...
  if (serverOwnerName) {
    ...
    return world.afterEvents.dataDrivenEntityTrigger.unsubscribe(e);
  }
